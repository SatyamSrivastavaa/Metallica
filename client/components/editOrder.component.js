import React, { Component } from "react";
import DropDown from './dropdown.component';
import axios from 'axios';
import { Link } from 'react-router';

export default class EditOrder extends Component {
    constructor() {
        super();
        this.state = {
            check: false
        }
    }

    editTrade() {
        let body = {
            "tradeId": this.props.params.id,
            "side": this.setBuySell(),
            "status": false,
            "quantity": this.refs.quantity.value,
            "price": this.refs.price.value,
            "date": this.refs.date.value,
            "commodity": this.refs.commSel.value,
            "counterParty": this.refs.cpSel.value,
            "location": this.refs.locSel.value
        }
        console.log(body)

        if (body.quantity <= 0 || body.price <= 0 || body.quantity.length==0 || body.price.length==0) {
            alert("Please fill form with correct data")
        } else {

            axios.put('http://localhost:8081/api/trades/update', body, {
                headers: { 'Content-Type': 'application/json' }
            }).then(dispatch => {
                dispatch(this.props.FetchTradeDetails());
            })
        }
    }
    
    setBuySell() {
        let buy = document.getElementById('buy_side').checked;
        var a;
        if (buy) {
            a = true;
        } else {
            a = false;
        }
        return a;
    }

    render() {
        var comms = this.props.myCommodities.map(
            (c, i) => {
                return <DropDown key={i} name={c.name} selectData={this.selectComm} {...this.props} />
            }
        )
        var cpList = this.props.myCounterParties.map(
            (cp, i) => {
                return <DropDown key={i} name={cp.name} selectData={this.selectCP} {...this.props} />
            }
        )
        var locList = this.props.myLocations.map(
            (l, i) => {
                return <DropDown key={i} name={l.cityname} selectData={this.selectLoc} {...this.props} />
            }
        )
        let tempId = this.props.params.id;
        var tempTrade = this.props.myTrades.find(
            (t) => {
                if (t.tradeId == tempId) {
                    return true;
                }
            }
        );
        return (
            <div>
                <div className="row" style={{ borderBottom: "solid 1px black" }}>
                    <span className="col-sm-6" style={{ paddingTop: "0.8%" }}><b>Edit Trade: {tempTrade.tradeId}</b></span>
                </div>
                <form style={{ paddingLeft: "1%", marginTop: "4%" }}>
                    <div className="form-group row">
                        <label for="tdate" className="col-sm-4 col-form-label" style={{ paddingTop: "8px" }}>Trade Date</label>
                        <div className="col-sm-8">
                            <input type="text" ref="date" value={tempTrade.date} className="form-control" id="tdate" />
                        </div>
                    </div>
                    <br />
                    <div className="row">
                        <label className="control-group col-sm-4" for="symbol" style={{ paddingTop: "8px" }}>Commodity: </label>
                        <div className="col-sm-8">
                            <select ref="commSel" className="dropdown">
                                {comms}
                            </select>
                        </div>
                    </div>
                    <br />

                    <fieldset className="form-group row">
                        <label className="form-check-label col-sm-4">Side: </label>
                        <div className="form-check form-check-inline col-sm-4">
                            <input className="form-check-input" ref="buy" type="radio" name="side" id="buy_side" value="1" />
                            <label className="form-check-label" for="buy_side">Buy</label>
                        </div>
                        <div className="form-check form-check-inline col-sm-4">
                            <input className="form-check-input" checked type="radio" name="side" id="sell_side" value="0" />
                            <label className="form-check-label" ref="sell" for="sell_side">Sell</label>
                        </div>
                    </fieldset>

                    <div className="row">
                        <label className="control-group col-sm-4" for="party" style={{ paddingTop: "8px" }}>Counterparty: </label>
                        <div className="col-sm-8">
                            <select ref="cpSel" >
                                {cpList}
                            </select>
                        </div>
                    </div>
                    <br />
                    <div className="form-group row">
                        <label for="price" className="col-sm-4 col-form-label" style={{ paddingTop: "8px" }}>Price: </label>
                        <div className="col-sm-6 input-group">
                            <span className="input-group-addon">$</span>
                            <input type="text" ref="price" className="form-control" id="price" />
                        </div>
                    </div>
                    <br />
                    <div className="form-group row">
                        <label for="qty" className="col-sm-4 col-form-label" style={{ paddingTop: "8px" }}>Quantity: </label>
                        <div className="col-sm-6">
                            <input type="text" className="form-control" ref="quantity" id="qty" />
                        </div>
                    </div>
                    <br />

                    <div className="row">
                        <label className="control-group col-sm-4" for="location" style={{ paddingTop: "8px" }}>Location: </label>
                        <div className="col-sm-8">
                            <select ref="locSel">
                                {locList}
                            </select>
                        </div>
                    </div>
                    <br />
                    <div className="form-group row col-sm-12">
                        <div>
                            <Link to={"/home/view/" + tempId} ><button type="button" className="btn btn-default" style={{ margin: "5%" }}>Cancel</button></Link>
                            <button type="button" onClick={this.editTrade.bind(this)} className="btn btn-default">Save</button>
                        </div>
                    </div>
                </form>
            </div>
        )

    }
}